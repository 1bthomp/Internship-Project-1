...
def main_page(context):
    ...
    # Search in user
    search_bar_1 = context.driver.find_element(By.ID, "email-2")
    search_bar_1.clear()
    search_bar_1.send_keys("Thompb155@gmail.com")
    sleep(4)

    # Search password
    search_bar_2 = context.driver.find_element(By.ID, "field")
    search_bar_2.clear()
...
def settings_subs_payment(context):
    # Click settings
    context.driver.find_element(By.LINK_TEXT, 'Settings').click()
    sleep(4)

    # Click subscription
    context.driver.find_element(By.CSS_SELECTOR, 'a[href="/subscription"]').click()
    sleep(4)

@then('Verify subscription, back button, and upgrade plan')
def verification(context):
    check_1 = context.driver.find_element(By.XPATH, "//div[@class='lotion-your-h3--price size']").text
    check_2 = context.driver.find_element(By.XPATH, "//div[@class='back-text']").text
    check_3 = context.driver.find_element(By.XPATH, "//div[@class='next-step--']").text
